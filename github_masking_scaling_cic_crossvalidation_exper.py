# -*- coding: utf-8 -*-
"""Github_Masking_Scaling_CIC_Crossvalidation_Exper.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1PTbcGXxU5tWKRZNqhfZs6ZdhQ5h8_NFi
"""
...
# Load datasets
train = pd.read_csv('cic_balanced_train_data.csv')
test = pd.read_csv('cic_balanced_test_data.csv')
...
results_file = "cis_original_evaluation_results.txt"
...
adversarial_loaders = {
    "PGD": "X_adv_pgd.csv",
    "JSMA": "X_adv_jsma.csv",
    "DeepFool": "X_adv_df.csv",
    "FGSM": "X_adv_fgsm.csv",
    "ZOO": "X_adv_zoo_agg.csv",
    "HSJ": "X_adv_hsj_cic.txt",
    "CW": "X_adv_cw_new.txt",
    "LPF": "X_adv_lpf.csv",
...
cw_attack = CarliniL2Method(
     ...
     binary_search_steps=5, # on cic it was 10
     initial_const=0.01,
     max_halving=3,
     max_doubling=3,
 )

pgd = ProjectedGradientDescent(estimator=classifier, eps=0.15, eps_step=0.01, max_iter=40)#aggressive attack with inf norm

deepfool = DeepFool(classifier, max_iter=10)

saliency = SaliencyMapMethod(classifier)

fgsm = FastGradientMethod(classifier, eps=0.15)#aggressive attack with inf norm
...
np.savetxt("X_adv_pgd_cic_M.txt", X_adv_pgd, delimiter=",")
print("Saved PGD adversarial examples to 'X_adv_pgd_cic_M_norm.txt'.")
...
np.savetxt("X_adv_fgsm_cic_M.txt", X_adv_fgsm, delimiter=",")
print("Saved FGSM adversarial examples to 'X_adv_fgsm_cic_M_norm.txt'.")
...
np.savetxt("X_adv_df_cic_M.txt", X_adv_df, delimiter=",")
print("Saved DeepFool adversarial examples to 'X_adv_df_cic_M.txt'.")
...
np.savetxt("X_adv_jsma_cic_M.txt", X_adv_jsma, delimiter=",")
print("Saved JSMA adversarial examples to 'X_adv_jsma_cic_M.txt'.")

import torch
import torch.nn as nn

# Wrap your existing model to force fp32 at the very start
class FP32Wrapper(nn.Module):
    ...
    def forward(self, x, *args, **kwargs):
        x = x.to(torch.float32)  # <- force inputs to float32
        return self.base(x, *args, **kwargs)

# Build a wrapped model (no retrain needed)
wrapped_model = FP32Wrapper(model).to(next(model.parameters()).device).eval()

from art.estimators.classification import PyTorchClassifier

classifier = PyTorchClassifier(
    model=wrapped_model,                 # <- use wrapper
    clip_values=(0, 1),
    loss=criterion,
    optimizer=optimizer,
    input_shape=(X_test_tensor.shape[1],),
...
# Make sure inputs to attack are float32 (good practice)
...
np.savetxt("X_adv_hsj_cic_M.txt", X_adv_hsj, delimiter=",")
print("Saved HSJ adversarial examples to 'X_adv_hsj_cic_M.txt'.")
...
np.savetxt("X_adv_cw_agg_cic_M.txt", X_adv_cw, delimiter=",")
...
X_adv_zoo = generate_adversarial_examples(zoo_attack, X_test_np, cat_idxs)
#X_adv_zoo = zoo_attack.generate(X_test_np)
np.savetxt("X_adv_zoo_agg_cic_M.txt", X_adv_zoo, delimiter=",")
...
# Convert original X_test to numpy float32 to restore teh catagorical
...
# Restore categorical columns from original test data
adversarial_examples[:, cat_idxs] = X_test_np[:, cat_idxs]

# Save all adversarial examples to a single file
np.savetxt("X_adv_lowprofool_cic_M_lessaggr.csv", adversarial_examples, delimiter=",")
...
# Testing on adversarial examples
adversarial_loaders = {
    "PGD": "X_adv_pgd_cic_M.txt",
    "JSMA": "X_adv_jsma_cic_M.txt",
    "DeepFool": "X_adv_df_cic_M.txt",
    "FGSM": "X_adv_fgsm_cic_M.txt",
    "ZOO": "X_adv_zoo_agg_cic_M.txt",
    "CW": "X_adv_cw_agg_cic_M.txt",
    "LPF": "X_adv_lowprofool_cic_M.csv",
    "HSJ": "X_adv_hsj_cic_M.txt"
...
results = train_and_evaluate_on_adversarial_folds(
    ...
    num_epochs=30,
    folds=5,
    output_file="results/sigma0.5_masking_less_cic_adaptive_smoothing.txt",
)
...
def train_and_evaluate_on_adversarial_folds(
    ...
    num_epochs=30,
    folds=5,
    output_file="up_cic_adversarial_evaluation_results_originalSmoothing.txt",
):
    import torch
    from torch.utils.data import DataLoader, TensorDataset, Subset
    from sklearn.model_selection import KFold
...
results = train_and_evaluate_on_adversarial_folds(
    ...
    num_epochs=30,
    folds=5,
    output_file="sigma0.5_masking_less_cic_originalSmoothing.txt",
)